...
window = Tk()
window.title("Hangman        Player: " + name)
        ...
        if the_word_withSpaces.count(letter) > 0:
            for c in range(len(txt)):
                ...
                if lblWord.get() == the_word_withSpaces:
                    ...
                    messagebox.showinfo("Hangman", "If you want to play more, press New Game, else close the window.")
                    break
        else:
            NumberOfGuesses += 1
...
imgLabel.grid(row = 0, column = 0, columnspan = 4, padx = 10, pady = 40)
imgLabel.config(image = photos[0])
